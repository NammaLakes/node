...
class LakewatchGUI:
    ...
    def setup_ui(self):
        ...
        ttk.Label(self.info_frame, textvariable=self.node_id_var, font=("Arial", 12)).pack(anchor=tk.W, padx=10, pady=2)
        
        self.location_var = tk.StringVar(value="Location: --")
        ttk.Label(self.info_frame, textvariable=self.location_var, font=("Arial", 12)).pack(anchor=tk.W, padx=10, pady=2)
        ...
        ttk.Label(self.temp_frame, text="Temperature:", font=("Arial", 14)).pack(side=tk.LEFT, padx=10)
        self.temp_var = tk.StringVar(value="-- °C")
        self.temp_label = ttk.Label(self.temp_frame, textvariable=self.temp_var, font=("Arial", 18, "bold"))
        ...
        ttk.Label(self.ph_frame, text="pH Level:", font=("Arial", 14)).pack(side=tk.LEFT, padx=10)
        self.ph_var = tk.StringVar(value="--")
        self.ph_label = ttk.Label(self.ph_frame, textvariable=self.ph_var, font=("Arial", 18, "bold"))
        ...
        ttk.Label(self.status_frame, text="Last Update:", font=("Arial", 12)).pack(side=tk.LEFT, padx=10)
        self.last_update_var = tk.StringVar(value="--")
        ttk.Label(self.status_frame, textvariable=self.last_update_var, font=("Arial", 12)).pack(side=tk.LEFT, padx=10)
        
        # Connection status
        self.connection_var = tk.StringVar(value="Status: Not connected")
        self.connection_label = ttk.Label(self.status_frame, textvariable=self.connection_var, font=("Arial", 12))
        ...
        # Clear button
        self.clear_btn = ttk.Button(self.control_frame, text="Clear Data", command=self.clear_data)
        self.clear_btn.pack(side=tk.LEFT, padx=5)
        
        # Exit button
        self.exit_btn = ttk.Button(self.control_frame, text="Exit", command=self.root.quit)
    ...
    def update_data(self):
        try:
            ...
            while not self.data_queue.empty():
                ...
                self.location_var.set(f"Location: {CONFIG.get('latitude', 0)}, {CONFIG.get('longitude', 0)}")
...
def start_gui(use_test_data=False):
    ...
    data_thread = threading.Thread(target=collector_func, args=(data_queue,), daemon=True)
    data_thread.start()
